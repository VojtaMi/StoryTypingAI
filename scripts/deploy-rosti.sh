#!/usr/bin/env bash
# Deploy this app to a Rosti *app* (Node + supervisor, no Docker).
# Background and the traps this works around: rosti/README.md.
#
...
# reading-openings/, openings/, learner/, and the audio and image caches)
# *inside* /srv/app, so a whole-directory `rsync --delete` would erase every
# saved story and force each cached provider call to be paid again. Syncing the
# build paths individually keeps --delete scoped to dist/.
set -euo pipefail

# Deliberately has no default: every Rosti app is a separate deployment, and a
# wrong id silently overwrites a different running app.
if [[ -z "${ROSTI_APP_ID:-}" ]]; then
  cat >&2 <<'USAGE'
Deployment stopped: ROSTI_APP_ID is not set.

Create an app in the Rosti web admin (the CLI has no `apps create`), find its
id with `rosticli apps list`, then:

  ROSTI_APP_ID=<id> bash scripts/deploy-rosti.sh

Do not reuse another project's app id — it would replace that deployment.
USAGE
  exit 1
fi
APP_ID="$ROSTI_APP_ID"
...
Create it once, with the provider keys (no PORT — the start script sets it):

  rosticli apps ssh --app-id $APP_ID -- sh -lc 'cat > /srv/app/.env' <<'EOF'
  OPENAI_API_KEY=...
  GEMINI_API_KEY=...
  ANTHROPIC_API_KEY=...
  EOF
  rosticli apps ssh --app-id $APP_ID -- chmod 600 /srv/app/.env
USAGE
  exit 1
fi
