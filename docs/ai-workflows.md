...
| **Next-story brief** | [`src/server/nextStoryBriefService.ts`](../src/server/nextStoryBriefService.ts) | Finalization distills the completed prose, recap, story-scoped lookups, learner questions, difficulty, and practice request into a broad theme suggestion, an absolute `minimal` or `simple` narrative scale, and one language plan. Calibration snippets may quote or lightly paraphrase the completed story, but must remain representative of its language difficulty. The same call describes the completed story as one compact anti-repetition record; code merges it into the newest-first five-story FIFO. Both the next-theme planner and plot author receive this curated memory. If the handoff is malformed, recovery retains the validated brief that generated the completed story, applies only the difficulty adjustment, leaves theme choice open, and preserves the FIFO. |
