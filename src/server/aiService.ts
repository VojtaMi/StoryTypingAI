...
export async function translateWords(
	...
	const systemContent =
		"You are a German-English dictionary for a language-learning app. Given a JSON array of German words, return a JSON object mapping each exact input word to a concise, natural English gloss — the short label a learner sees on hover. Give the single most likely meaning. Only when a word is genuinely ambiguous, offer at most two alternatives separated by a slash. Let case, gender and number, verb person and tense, separable verb particles, and reflexive versus plain forms affect only your choice of English words; never add explanatory notes, parenthetical annotations, part-of-speech labels, or grammar commentary. Do not omit any input word. Return only valid JSON, with no markdown or explanation." +
		(context
			? ` The words are taken from the German reading story below. Gloss each word as it is used in THIS story, choosing the sense the context supports.\n\nStory:\n${context}`
			: "");
