...
const wordConnectSpec: RecapExerciseSpec<StoryRecapWordConnectExercise> = {
	shape: {
		pairs: [{ term: "German word", meaning: "English meaning" }],
	},
	instructions:
...
const fillMissingWordSpec: RecapExerciseSpec<StoryRecapFillMissingWordExercise> =
	{
		shape: {
			sentence:
				"A complete, natural German sentence that contains the answer word",
			answer: "correct German word, exactly as it appears in sentence",
		...
		instructions:
			...
			"Choose the fill sentence so its blanked `answer` is the exact German word or short phrase that realizes that focus. " +
			"The `answer` is a single word or at most a short two-word phrase (e.g. `steht auf`). " +
			"The fill sentence must be one complete, natural German sentence containing the answer written exactly as in `answer` — the app carves the blank out of it itself, so write a normal sentence and do not pre-split it or omit the answer.",
		parse(value) {
...
export function buildStoryRecapPrompt(primaryFocus?: string): string {
	...
	return (
		"Create a tiny end-of-story German recap lesson for a beginner. " +
