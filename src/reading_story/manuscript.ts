...
export interface ReadingStoryManuscript {
	title: string;
	/** Immutable finished Spanish prose. Later stages may divide but never rewrite it. */
	text: string;
}

const MANUSCRIPT_SHAPE = {
	title: "short Spanish title, 2-6 words",
	text: "the complete uninterrupted Spanish story",
};

const LENGTH_GUIDANCE: Record<NextStoryBrief["narrativeScale"], string> = {
	minimal:
		"Write roughly 90-150 Spanish words in 8-14 short sentences. Let a genuinely tiny plot finish sooner rather than padding it.",
	simple:
		"Write roughly 160-260 Spanish words in 12-22 short sentences. Let the reviewed plot determine the exact length.",
};
...
> = {
	...
	harder:
		"Make the Spanish modestly richer than the calibration snippets through natural sentence variety and the stated language focus. Do not increase plot density, introduce unrelated grammar targets, or make individual sentences difficult merely to signal progression.",
};

const MANUSCRIPT_PROMPT = `Author the finished manuscript for one beginner Spanish reading story: a concise Spanish title and the final uninterrupted prose.
...
- Prefer natural, clear Spanish over inserting unrelated vocabulary or constructions. Avoid unrelated advanced grammar.
- Keep the Spanish grammatical at every level. Maintain noun/adjective gender and number agreement, correct verb conjugation, and correct use of ser and estar. Never simplify by producing ungrammatical Spanish. If the focus says to avoid a construction, choose sentences that do not require it.
- Write every narrative and dialogue word in Spanish except proper names. Convey meaning through the situation; do not insert English glosses or translations into the story.
...
Preserve the complete Spanish prose. Fix only invalid JSON, a missing concise title, or a missing text wrapper. Do not divide, summarize, continue, or rewrite valid prose. Return JSON only.`;
