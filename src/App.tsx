...
import StoryView, { type StorySegment, type StoryPhase } from './StoryView'
import { startStory, continueStory, type ChatMessage } from './ai'
...
export default function App() {
  ...
  function handleTypingComplete(_stats: TypingStats) {
    ...
    setSegments((prev) => [...prev, { author: 'ai', text: currentTarget }])
    setCurrentTarget(null)
    setPhase('authoring')
  }

  async function submitContinuation(userText: string) {
    setSegments((prev) => [...prev, { author: 'user', text: userText }])
    setError(null)
    setPhase('loading')
    try {
    ...
    <div className="app">
      <header className="header">
        ...
        <p className="subtitle">
          {view === 'story' && genre ? `${genre.emoji} ${genre.label}` : 'An interactive AI story'}
        </p>
      </header>
